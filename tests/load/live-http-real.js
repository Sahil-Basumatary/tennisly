...
const WRITE_URL = (__ENV.MATCH_WRITE_URL || __ENV.BASE_URL || 'http://127.0.0.1:18096').replace(/\/$/, '');
const BASE_URL = (__ENV.BASE_URL || WRITE_URL).replace(/\/$/, '');
const RUN_ID = __ENV.RUN_ID || `${Date.now()}`;
const CLIENTS = Number(__ENV.HTTP_LIVE_CLIENTS || 100);
const HOLD_S = Number(__ENV.HTTP_LIVE_HOLD_S || 30);
...
const POINT_INTERVAL_MS = Number(__ENV.POINT_INTERVAL_MS || 15000);
...
export function setup() {
  const homeId = uuid('a000', 1);
  const awayId = uuid('b000', 1);
  const create = http.post(
    `${WRITE_URL}/api/matches`,
    JSON.stringify({
      externalId: `http-live-${RUN_ID}`,
      surface: 'HARD',
      bestOfSets: 3,
      metadata: { benchmark: 'http-live', runId: RUN_ID },
      players: [
        { playerId: homeId, displayName: 'Home', side: 'HOME' },
        { playerId: awayId, displayName: 'Away', side: 'AWAY' },
      ],
    }),
  ...
  const status = http.patch(
    `${WRITE_URL}/api/matches/${matchId}/status`,
...
export function writer(data) {
  let n = 0;
  while (true) {
    const playerId = n % 2 === 0 ? data.homeId : data.awayId;
    const point = http.post(
      `${WRITE_URL}/api/matches/${data.matchId}/points`,
      JSON.stringify({
        serverId: playerId,
        winnerId: playerId,
        outcome: 'WINNER',
        rallyLength: 4,
        scoreSnapshot: { points: ['0', '0'], n },
        shotSummary: {},
      }),
      { headers: { 'Content-Type': 'application/json' }, tags: { name: 'writer' } },
    );
    check(point, { 'point committed': (res) => res.status === 201 });
    n += 1;
...
export function viewer(data) {
  ...
  while (true) {
    ...
    const live = http.get(`${BASE_URL}/api/matches/${matchId}/cursor`, { headers, tags: { name: 'cursor' } });
    ...
    if (live.status === 200) {
      ...
      if (needsEventRecovery(cursor, sequence)) {
        ...
        const events = http.get(
          `${BASE_URL}/api/matches/${matchId}/events?afterSequence=${cursor}&limit=1000`,
